...
angular.module('dianApp')
  ...
  .config(['$stateProvider', function ($stateProvider) {
    $stateProvider.state('console.overview', {
      url: '/overview',
  ...
  .controller('OverviewCtrl', ['$scope', '$http', '$cookies', '$state', '$stateParams', '$timeout', '$modal',
    ...
    $scope.make_repast = function(registration) {
      $http.get(config.api_url + '/registration/make-repast/' + registration.id + '/')
      .success(function(data, status, headers, config){
        init_detail();
      })
    ...
    $scope.make_expired = function(registration) {
      $http.get(config.api_url + '/registration/make-expired/' + registration.id + '/')
      .success(function(data, status, headers, config){
        init_detail();
      })
      .error(function(data, status, headers, config){
    ...
    //     // 当前号reg的状态设置
    //     $http.put(config.api_url + '/registration/registration/' + table_type.current_registration.id + '/', {
    //       "status": action
    //     })
    //     .success(function(data, status, headers, config){
    //     })
    ...
    //     $http.put(config.api_url + '/registration/registration/' + header_reg.id + '/', {
    //       "status": "turn"
    //     })
    ...
    //         $http.post(config.api_url + '/registration/msg-task/', {
    ...
    $scope.status_change = function(table, status) {
      $http.put(config.api_url + '/table/table/' + table.id + '/', {
        "status": status
      })
      .success(function (data, status, headers, config) {
    ...
    $scope.reset = function(){
      ...
      reset_modal.result.then(function (data) {
        angular.forEach($scope.table_types, function(table_type){
          $http.put(config.api_url + '/table/table-type/' + table_type.id + '/', {
            "next_queue_number": 1
          })
          .success(function (data, status, headers, config) {
          })
